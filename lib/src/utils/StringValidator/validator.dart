...
bool isInValidOldPass(String str) {
  return str.isEmpty || str.length > 3;
}

bool isInValidPass(String str) {
  return str.isEmpty || str.length > 5;
}

// must refactor
String isInValidPassMassage(String str) {
  return str.isEmpty
      ? '${tr('please_enter')} ' + tr('confirm_password').toLowerCase()
      : (str.length > 5) ? '${tr('pass_validation_error_msg')} ' : null;
}
...
bool isSaudiMobile(String value) {
  String pattern = r'(?:\+?0*?966)?0?5[0-9]{8}';
   RegExp regExp = new RegExp(pattern);
  return regExp.hasMatch(value) ;
}
...
bool isURL(String str, [Map options]) {
  ...
  var protocol, user, pass, auth, host, hostname, port, portStr, path, query, hash, split;
...
bool isFQDN(str, [options]) {
  Map defaultFqdnOptions = {'require_tld': true, 'allow_underscores': false};
